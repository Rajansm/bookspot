 ...
 * @param id
 */
const getBook = async (id: string): Promise<object> => {
  try {
    const user: any = await Book.findById(id);
    return {
      ...user._doc,
      _id: user.id,
      createdAt: dateToString(user._doc.createdAt),
 ...
 * @param book
 */
 const transformBook = async (book: any): Promise<object> => {
  return {
    ...book._doc,
    _id: book.id,
    user: getBook(book.userId),
    createdAt: dateToString(book._doc.createdAt),
    updatedAt: dateToString(book._doc.updatedAt)
  };
};

export { getUser, getNotice, getUserNotices, transformUser, transformNotice, transformBook };